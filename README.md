...
3. Config local ```MONGODB_URI``` and ```PORT``` in ```.env```file. At root directory, ```touch .env``` to create ```.env``` file, then add two lines of code```MONGODB_URI=mongodb://localhost:27017/tweeter/r\r``` ```PORT=8080```
